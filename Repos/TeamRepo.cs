    ...
    public class TeamRepo : ITeamRepository
    {
        ...
        public async Task<IEnumerable<Team>> GetAllTeamsByMember(string userId)
        {
            var teamMembers = await context.TeamsMembers
                                                .Include(tm=> tm.Team)
                                                .ThenInclude( tm => tm.Leader)
                                                .Where(tm => tm.UserId == userId).ToListAsync();        

            var teams = teamMembers.Select( tm => tm.Team).ToList();   
            return teams;
        ...
        public async Task<IEnumerable<Team>> GetAllTeamsByRoom(int roomId)
        {
           return( await context.Teams
                                    .Where(t => t.RoomId == roomId && t.IsActive == true)
        ...
        public async Task<Team> UpdateTeamById(int teamId,Team team)
        {
            ...
            if( NewTeam!=null && NewTeam.IsActive == true)
            {
        ...
        {
           return await context.Teams.Include( t => t.Leader).Where(t => t.LeaderId == userId && t.RoomId == roomId).ToListAsync();
        }
    }
}
