...
namespace work_platform_backend.Services
{
  ...
  public async Task<Team> AddTeam(Team newTeam,int roomId,string authUserId,int parentTeamId)
        ...
        public async Task<IEnumerable<TeamDto>> GetTeamsByRoom(int RoomId)
        {
            ...
            var TeamResponse = mapper.Map<IEnumerable<TeamDto>>(Teams);
        ...
        public async Task<List<UserDto>> GetMembersOfTeamsInRoom(int RoomId)
        {
            List<UserDto> usersInRoom = new List<UserDto>();
            var TeamsInRoom = await GetTeamsByRoom(RoomId);
            if (TeamsInRoom.Count() != 0)
            {
                foreach (TeamDto t in TeamsInRoom)
                {
                    
                  var membersofteams = await GetMembersOfTeam(t.Id);
                    usersInRoom.AddRange(membersofteams);
                }
        ...
        public async Task DeactivateTeam(int teamId)
        {
            var team = await teamRepository.GetTeamOnlyById(teamId);
            ...
            team.IsActiveTimeStamp = DateTime.Now;
            await teamRepository.SaveChanges();
        ...
        public async Task DeleteTeam(int teamId)
        {
            ...
            {
                Console.WriteLine("tasks in team ==============> " + team.Tasks.Count);
